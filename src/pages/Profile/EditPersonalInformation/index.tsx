...
export default function EditPersonalInformationComponent () {
    const {activeUserInformation, credentials} = useAccountContext();
    ...
    const onUpdateUserPersonalInformation = (values: UpdateProfileDTO) => {
        sendUpdateInfromationOfQRUser(activeUserInformation?.account?.fbID ?? '', values);
    };

    return <S.PersonalInformationContainer>
            ...
            <Formik initialValues={initValues} onSubmit={values => {
                ...
                {({handleSubmit, handleChange, values}) => (
                    <>
                        ...
                        <TextInputComponent label="Firstname" textMode={TextInputEnum.OUTLINED} value={values.firstname}
                        onChangeText={handleChange('firstname')} />
                        <DividerComponent margin="10px 0 0 0" />
                        <TextInputComponent label="Middlename" textMode={TextInputEnum.OUTLINED} value={values.middlename}
                        onChangeText={handleChange('middlename')} />
                        <DividerComponent margin="10px 0 0 0" />
                        <TextInputComponent label="Lastname" textMode={TextInputEnum.OUTLINED} value={values.lastname}
                        onChangeText={handleChange('lastname')} />
                        <DividerComponent margin="10px 0 0 0" />
                        <TextInputComponent label="Mobilenumber" textMode={TextInputEnum.OUTLINED} value={values.mobilenumber}
                        onChangeText={handleChange('mobilenumber')} />
                        <DividerComponent margin="10px 0 0 0" />
                        <TextInputComponent label="Pass****" textMode={TextInputEnum.OUTLINED} value={formatPasswordDisplay(values.password)}
                        onChangeText={handleChange('password')} secureTextEntry />
                        <DividerComponent margin="10px 0 0 0" />
                        <ButtonComponent title="Save" backgroundColor={COLOR_LISTS.RED_500} width={100} borderRadius="5" textAlign="center" padding="10" textColor={COLOR_LISTS.WHITE} fontSize={20}
